...
void UBTService_DetectPlayerCharacter::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	ADAIController* AIC = Cast<ADAIController>(OwnerComp.GetOwner());
	if (true == ::IsValid(AIC)) 
	{
		ADNonPlayerCharacter* NPC = Cast<ADNonPlayerCharacter>(AIC->GetPawn());
		if (true == ::IsValid(NPC))
		{
			UWorld* World = Cast<UWorld>(NPC->GetWorld());
			if (true == ::IsValid(World))
			{
				...
				if (true == bResult)
				{
					...
					{
						ADCharacterBase* PC = Cast<ADCharacterBase>(OverlapResult.GetActor());
						if (true == ::IsValid(PC))
						{
							if (true == PC->GetController()->IsPlayerController())
							{
								OwnerComp.GetBlackboardComponent()->SetValueAsObject(ADAIController::TargetActorKey, PC);
							...
							{
								OwnerComp.GetBlackboardComponent()->SetValueAsObject(ADAIController::TargetActorKey, nullptr);
